...
Evaluator.prototype.visitExtend = function(extend){
  ...
  // To avoid extend hoisting up out of the
  // media block, we create a parent reference
  // group here.
  group = new nodes.Group();
  group.lineno = extend.lineno;
  group.column = extend.column;
  group.filename = extend.filename;

  selector = new nodes.Selector([new nodes.Literal('&')]);
  selector.lineno = extend.lineno;
  selector.column = extend.column;
  selector.filename = extend.filename;
  selector.val = '&';
  selector.block = new nodes.Block(block, group);

  group.push(selector);
  block.push(group);

  block = group.block;
  block.lineno = extend.lineno;
  block.column = extend.column;
  block.filename = extend.filename;
...
/**
 * Return the closest media block.
 *
 * @return {Media}
 * @api private
 */

Evaluator.prototype.__defineGetter__('closestMedia', function(){
  var i = this.stack.length
    , block;
  while (i--) {
    block = this.stack[i].block;
    if (block.node && 'media' == block.node.nodeName) {
      return block;
    }
  }
});
