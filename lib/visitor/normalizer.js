...
Normalizer.prototype.bubble = function(node){
  var props = []
    , other = []
    , blockLength = node.block.nodes.length
    , self = this;
  ...
  if (props.length) {
    ...
    // Add any nodes that we added while
    // visiting this block in filterProps.
    // We assume anything added doesn't need
    // to be visited (they were already visited).
    for (var i = blockLength, il = node.block.nodes.length; i < il; i++) {
      other.push(node.block.nodes[i]);
    }
...
Normalizer.prototype.extend = function(group, selectors){
  var map = this.map
    ...
    , recentAtrule = this.getRecent(nodes.Atrule);
  ...
  group.extends.forEach(function(extend){
    ...
    selectors.forEach(function(selector){
      var node = new nodes.Selector([new nodes.Literal(selector)]);
      node.val = selector;
      node.inherits = false;

      groups.forEach(function(groupExtendedTo){
        var localNode;
        // prevent recursive extend
        if (!parent || (parent != groupExtendedTo)) self.extend(groupExtendedTo, selectors);

        if (recentAtrule) {
          localNode = node.clone();
          groupExtendedTo = groupExtendedTo.clone();
          localNode.block = groupExtendedTo.block;
          groupExtendedTo.nodes = [];
          groupExtendedTo.push(localNode);
          recentAtrule.block.push(groupExtendedTo);
        } else {
          groupExtendedTo.push(node);
        }
      });
    });
  });
