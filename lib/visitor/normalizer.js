...
Normalizer.prototype.extend = function(group, selectors){
  var map = this.map
    ...
    , recentMedia = this.getRecent('media');;
  ...
  group.extends.forEach(function(extend){
    ...
    selectors.forEach(function(selector){
      var node = new nodes.Selector([new nodes.Literal('&')]);
      node.val = selector;
      // Is this safe?
      node.segments = [new nodes.Literal(selector)];
      node.inherits = false;
      ...
      groups.forEach(function(group){
        ...
        function replaceReferenceNode(haystack, needle, cloneGroup) {
          for (var i = 0, il = haystack.nodes.length; i < il; i++) {
            if (haystack.nodes[i] === needle) {
              haystack.nodes.splice(i, 1, cloneGroup);

              return true;
            }

            if (haystack.nodes[i].nodes) {
              if (replaceReferenceNode(haystack.nodes[i], needle, cloneGroup) === true) {
                return true;
              }
            } else if (haystack.nodes[i].block) {
              if (replaceReferenceNode(haystack.nodes[i].block, needle, cloneGroup) === true) {
                return true;
              }
            }
          }
        ...
        if (recentMedia || extend.media) {
          var clone = group.clone();

          for (var i = 0 , il = clone.nodes.length; i < il; i++) {
            clone.nodes[i].val = '&';
            clone.nodes[i].segments = [new nodes.Literal('&')];
          }

          replaceReferenceNode(recentMedia ? recentMedia.block : extend.media, extend.referenceNode, clone);
        } else {
          group.push(node);
        }
      });
  ...
  if (false && recentMedia) {
    //recentMedia.block = this.visit(recentMedia.block);
  } else {
